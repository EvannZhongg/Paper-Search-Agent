...
- `query_hints` 已收敛为 provider-friendly 检索短语，不再把自然语言提示句直接送进 query bundle
- 组合条件 `c4` 已保留独立 query 位，并参与 `criteria-and` 合取查询
...
- 当前 Deep 召回已切到面向复杂组合查询的 query bundle，优先使用 `intent.rewritten_query`，再回退原始 query，并补充 criteria-focused query
- 组合条件 `c4` 已进入合取查询与独立 criterion query，且 query phrase 会先清洗成 provider-friendly 检索短语
- 每个检索源内会先做 criterion-level heuristic 预评分与 required coverage 计算
- 已有基础硬过滤
- 在有可用 LLM 配置时，可对每个检索源内的 Top-N 候选逐篇做 criterion-level 结构化 judge
...
- Deep 召回不再只依赖少量 query variants，而是生成 query bundle，包括主合取查询、放宽查询、同义改写与原始 query fallback
- query bundle 中的 criterion phrase 已优先压缩成 provider-friendly 检索短语，避免把整句 prompt 直接送入 provider
- 组合条件 `c4` 已保留独立 query 位，并进入合取查询，而不是只在 judge 阶段存在
...
- `query_hints` 已压缩为 provider-friendly 检索短语，prompt 化 bundle 已明显减少
- 组合条件 `c4` 已保留独立 query 位，并真正参与合取召回
- 组合条件支持阈值已调高，coverage 不再被 heuristic/LLM 的 `or + max` 融合虚高抬满

基于 2026-03-30 对“找一些有关文本 rag 和图 rag 相结合的论文”的最新 smoke test，可以确认：

- GraphRAG-only 候选已不再普遍被打成满 coverage，明显错误项也能被压到 `drop`
- 当前 Top 结果已经更接近“真正结合 text RAG + graph RAG 的论文候选”
- 但输出仍更像“高相关候选集”而不是“很干净的最终结果”，同一轮返回仍有 47 条，且 Top 结果里还有 `hybrid evidence retrieval`、`multimodal RAG`、`graph-enhanced RAG` 这类边界命中
...
- 继续清洗 planner 产出的 criterion hints 和 `criteria-and` phrase，减少 prompt 化、重复和串味 query 进入 query bundle
...
3. 继续收敛已经落地的 `deep`，优先补齐最终 hard prune、`2-of-4 / 3-of-4` 放宽组合、更严格的 `c4` 证据要求、动态送审窗口和更稳定的 criterion-level evidence
